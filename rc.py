...
from utils.tokenization import tokenize_rc_sdd
...
def tokenize(examples, tokenizer, ds_factor, buffer_ds_factor=0):
    ...
    for i in range(len(examples['review_headline'])):
        src = examples['review_headline'][i]
        tgt = examples['review_body'][i]
        ...
        encoded['labels'] += [examples['star_rating'][i]-1]
...
if __name__ == "__main__":
    ...
    dataset = {}
    dataset['train'] = load_dataset('dataloading/amazon_rc_dataset.py', 'Books_v1_00', cache_dir=args.cache_dir, split='train[:99%]')
    dataset['validation'] = load_dataset('dataloading/amazon_rc_dataset.py', 'Books_v1_00', cache_dir=args.cache_dir, split='train[99%:]')
    dataset['test'] = load_dataset('dataloading/amazon_rc_dataset.py', 'Books_v1_02', cache_dir=args.cache_dir, split='train[:5%]')
    ...
    dataset['train'] = dataset['train'].filter(lambda example: len(example['review_headline']) + len(example['review_body']) + 1 < 256, num_proc=10)
    dataset['validation'] = dataset['validation'].filter(lambda example: len(example['review_headline']) + len(example['review_body']) + 1 < 256, num_proc=10)
    dataset['test'] = dataset['test'].filter(lambda example: len(example['review_headline']) + len(example['review_body']) + 1 < 256, num_proc=10)
    ...
    if args.model_type == "sdd":
        tokenize = partial(tokenize_rc_sdd, char_tokenizer=args.tok, word_tokenizer=args.word_tok)
    ...
    for split in dataset:
        dataset[split] = dataset[split].map(tokenize, batched=True, num_proc=10, remove_columns=[
            "customer_id", "helpful_votes", "marketplace", "product_category", "product_id", "product_parent", 
            "product_title", "review_body", "review_date", "review_headline", "review_id", "star_rating", 
            "total_votes", "verified_purchase", "vine", 
        ], load_from_cache_file=not args.recache_tok)
